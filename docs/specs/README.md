...
- Authority: epoch `tz-v16` and pinned digest declared below
...
Status: active  
Contract epoch: `tz-v16`
Local specification revision: `spec-19`
...
Release baseline: `0.5.0 released`; immutable historical baseline: `0.4.0`; website baseline: canonical domain released; artifact-hygiene workflow candidate unreleased
...
1. the user-approved ТЗ at epoch `tz-v1`, its pinned digest, the explicit addenda through `INSTALL-UX-001` and `ARTIFACT-HYGIENE-001` that advance the combined contract to `tz-v16`, and the behavior-preserving `REALISTIC-FIXTURES-001` acceptance addendum;
...
| --- | --- | --- | --- | --- | --- |
| [`product-contract.md`](product-contract.md) | Goal, scope, invariants, LLM boundary, operations, safety, non-goals, workflow routing | Normative restatement of ТЗ plus authorized addenda through `ARTIFACT-HYGIENE-001` | active / released CLI baseline plus evolving workflow hygiene | Any product decision or behavior claim | 0.5.0 + unreleased workflow candidate |
...
| [`acceptance.md`](acceptance.md) | Fixtures, determinism, skills, CI, Definition of Done | Normative acceptance map for ТЗ §§44–51 plus authorized addenda through `ARTIFACT-HYGIENE-001` | active | Implementing or verifying | 92 tests and hosted CI accepted; workflow-hygiene candidate pending local acceptance |
