...
1. the user-approved ТЗ at epoch `tz-v1`, its pinned digest, the explicit addenda through `RELEASE-0.6.1-001` that advance the combined contract to `tz-v24`, and the behavior-preserving `REALISTIC-FIXTURES-001` acceptance addendum;
...
Advance the epoch before accepting a material semantic change. Current authority is `tz-v24`; editorial clarification may advance only the local specification revision and must preserve every protected behavior and exception.
