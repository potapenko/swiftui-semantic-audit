...
Local specification revision: `spec-6`
...
1. the user-approved ТЗ at epoch `tz-v1`, its pinned digest, the explicit `ROUTER-001`, `INDEXED-SKILLS-001`, `BOUNDARY-001`, and `ARCHITECTURE-001` requests that advance the combined contract to `tz-v5`, and the behavior-preserving `REALISTIC-FIXTURES-001` acceptance addendum;
