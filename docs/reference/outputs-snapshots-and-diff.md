...
| Live semantic state | Holds the watcher's latest provisional and indexed generations. | External application state; available in version `0.6.0` and agent-usable only with a matching fresh indexed status receipt. |
...
Release `0.5.0` remains the immutable on-demand-snapshot baseline. Version
`0.6.0` preserves that path and adds watcher-backed live semantic state. Agents
may use watcher state only with a matching fresh indexed status receipt; stale
or syntax-preview state is never accepted.
