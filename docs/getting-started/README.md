...
An agent workflow requires compiler-backed indexed evidence. With the project
watcher, use a live snapshot only with the matching status receipt that reports
`fresh: true`, `resolution: "indexed"`, equal `workspaceDigest` and
`indexedWorkspaceDigest`, and the expected `configurationDigest`. When that
receipt is unavailable, use the explicit Index Store path:
