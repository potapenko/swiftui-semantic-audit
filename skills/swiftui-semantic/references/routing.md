...
This reference applies only in strict mode. Implicit assist mode follows
[`assist-mode.md`](assist-mode.md) and never enters this specialist matrix.

## Decision matrix

| Request state and outcome | Select first | Continue when needed |
| --- | --- | --- |
| Semantic investigation with no trusted evidence | `swiftui-semantic-audit` | Stop after the report unless the user requested a change |
